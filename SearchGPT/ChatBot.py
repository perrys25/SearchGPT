...
class ChatBot:
    def __init__(self, name, commands = [SearchCommand()], type="text-based assist chatbot",
        ...
        self.prompt = prompt.replace("{name}", name).replace("{type}", type).replace("{purpose}", purpose).replace("{commands}",  commandDescriptions) + "\n\n"
        ...
        while conversation[-1]["type"] != "Response":
            ...
            for command in self.commands:
                if conversation[-1]["type"] == command.getName():
                    result = command.execute(self, conversation[-1]["value"], question.replace("[", "").replace("]", ""))
            ...
            if not complete and not keepGoing:
                break
